...
package com.vstecs.android.funframework.netstatus;
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (intent.getAction().equalsIgnoreCase(ANDROID_NET_CHANGE_ACTION) || intent.getAction().equalsIgnoreCase(CUSTOM_ANDROID_NET_CHANGE_ACTION)) {
            if (!NetUtils.isNetworkAvailable(context)) {
                KJLoger.debug(TAG, "<--- network disconnected --->");
                isNetAvailable = false;
            } else {
                KJLoger.debug(TAG, "<--- network connected --->");
                isNetAvailable = true;
    ...
    public static void unRegisterNetworkStateReceiver(Context mContext) {
        if (mBroadcastReceiver != null) {
            try {
                ...
                KJLoger.debug(TAG, e.getMessage());
            }
        }
